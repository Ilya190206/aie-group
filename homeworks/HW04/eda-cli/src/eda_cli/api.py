...
# ---------- Модель для нового эндпоинта ----------


class QualityFlagsResponse(BaseModel):
    """Полный набор флагов качества данных с детальной информацией."""
    
    flags: Dict[str, Any] = Field(
        ...,
        description="Все флаги качества, включая новые эвристики из HW03"
    )
    metadata: Dict[str, Any] = Field(
        ...,
        description="Метаданные обработки файла"
    )
...
async def quality_from_csv(file: UploadFile = File(...)) -> QualityResponse:
    ...
    # Базовые флаги
    base_flags = ["too_few_rows", "too_many_columns", "too_many_missing"]
    for flag in base_flags:
        if flag in flags_all:
            flags_bool[flag] = bool(flags_all[flag])
    
    # НОВЫЕ ФЛАГИ ИЗ HW03 - ВАЖНО: они теперь будут включены в ответ
    new_flags = [
...
# ---------- НОВЫЙ ЭНДПОИНТ: /quality-flags-from-csv (HW04) ----------
...
@app.post(
    ...
    tags=["quality", "hw04"],
    summary="Полный набор флагов качества данных с детальной информацией",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    """
    Эндпоинт возвращает полный набор флагов качества, включая новые эвристики из HW03.
    Включает детальную информацию о проблемных колонках.
    """
    ...
    # Используем EDA-ядро из S03/HW03
    summary = summarize_dataset(df)
    missing_df = missing_table(df)
    ...
    # Формируем полный ответ со всеми флагами
    response_flags = {}
    
    # 1. Базовые булевы флаги
    for key, value in quality_flags.items():
        if isinstance(value, bool):
            response_flags[key] = value
    
    # 2. Числовые значения (quality_score, max_missing_share и др.)
    for key in ["quality_score", "max_missing_share"]:
        if key in quality_flags:
            response_flags[key] = quality_flags[key]
    
    # 3. Детальная информация о проблемных колонках (новые эвристики из HW03)
    if quality_flags.get("has_constant_columns"):
        response_flags["constant_columns_details"] = {
            "count": quality_flags.get("constant_columns_count", 0),
            "columns": quality_flags.get("constant_columns_list", [])
        }
    
    if quality_flags.get("has_high_cardinality_categoricals"):
        response_flags["high_cardinality_details"] = {
            "count": quality_flags.get("high_cardinality_count", 0),
            "columns": quality_flags.get("high_cardinality_columns", [])
        }
    
    if quality_flags.get("has_suspicious_id_duplicates"):
        response_flags["suspicious_id_details"] = {
            "count": quality_flags.get("suspicious_id_count", 0),
            "columns": quality_flags.get("suspicious_id_columns", [])
        }
    
    if quality_flags.get("has_many_zero_values"):
        response_flags["high_zero_details"] = {
            "count": quality_flags.get("high_zero_count", 0),
            "columns": quality_flags.get("high_zero_columns", [])
        }

    # 4. Метаданные для логирования
    metadata = {
        ...
        "has_missing": not missing_df.empty,
    }

    # Логирование
    print(
        f"[quality-flags-from-csv] filename={file.filename!r} "
        f"n_rows={metadata['n_rows']} n_cols={metadata['n_cols']} "
        f"score={response_flags.get('quality_score', 0):.3f} "
        f"latency_ms={latency_ms:.1f} ms"
    )
