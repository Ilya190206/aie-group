...
def report(
    ...
    - top-k категорий по категориальных признаках;
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        # ДОБАВЛЕНО: Вывод деталей новых эвристик
        if quality_flags.get('has_constant_columns'):
            const_cols = quality_flags.get('constant_columns_list', [])
            count = quality_flags.get('constant_columns_count', 0)
            f.write(f"- Константные колонки: **ДА** ({count} шт.)")
            if const_cols:
                f.write(f": {', '.join(const_cols[:3])}")
                if len(const_cols) > 3:
                    f.write(f" и ещё {len(const_cols) - 3}")
            f.write("\n")
        
        if quality_flags.get('has_high_cardinality_categoricals'):
            high_card_cols = quality_flags.get('high_cardinality_columns', [])
            count = quality_flags.get('high_cardinality_count', 0)
            f.write(f"- Высокая кардинальность категориальных: **ДА** ({count} шт.)\n")
            if high_card_cols:
                for col_info in high_card_cols[:2]:
                    col_name = col_info.get('column', 'N/A')
                    unique_count = col_info.get('unique_count', 0)
                    f.write(f"  - {col_name}: {unique_count} уникальных значений\n")
                if len(high_card_cols) > 2:
                    f.write(f"  - ... и ещё {len(high_card_cols) - 2} колонок\n")
        
        if quality_flags.get('has_suspicious_id_duplicates'):
            suspicious_cols = quality_flags.get('suspicious_id_columns', [])
            count = quality_flags.get('suspicious_id_count', 0)
            f.write(f"- Подозрительные дубликаты ID: **ДА** ({count} шт.)\n")
            if suspicious_cols:
                for col_info in suspicious_cols[:2]:
                    col_name = col_info.get('column', 'N/A')
                    uniqueness = col_info.get('uniqueness_ratio', 0)
                    f.write(f"  - {col_name}: уникальность {uniqueness:.1%} (ожидается >99.9%)\n")
                if len(suspicious_cols) > 2:
                    f.write(f"  - ... и ещё {len(suspicious_cols) - 2} колонок\n")
        
        if quality_flags.get('has_many_zero_values'):
            zero_cols = quality_flags.get('high_zero_columns', [])
            count = quality_flags.get('high_zero_count', 0)
            f.write(f"- Много нулевых значений: **ДА** ({count} шт.)\n")
            if zero_cols:
                for col_info in zero_cols[:2]:
                    col_name = col_info.get('column', 'N/A')
                    zero_ratio = col_info.get('zero_ratio', 0)
                    f.write(f"  - {col_name}: {zero_ratio:.1%} нулей\n")
                if len(zero_cols) > 2:
                    f.write(f"  - ... и ещё {len(zero_cols) - 2} колонок\n")
